...
def get_flag_reason(df: pd.DataFrame, thresholds: dict) -> pd.Series:
    ...
    """
    conditions = [
        (df['VIF'] >= thresholds['vif_strict']) & (df['image_outlier_percentage'] >= thresholds['outlier_strict']),
        df['VIF'] >= thresholds['vif_strict'],
        df['image_outlier_percentage'] >= thresholds['outlier_strict'],
        (df['VIF'] >= thresholds['vif_combined']) & (df['image_outlier_percentage'] >= thresholds['outlier_combined'])
    ]
    reasons = [
...
def summarize_flagged_scans(flagged_df: pd.DataFrame):
    ...
    contrast_counts = flagged_df.groupby('contrast_name').size().reset_index(name='flagged_scan_count')
    contrast_counts = contrast_counts.sort_values(by='flagged_scan_count', ascending=False)
    
    total_flags = contrast_counts['flagged_scan_count'].sum()
    total_row = pd.DataFrame([{'contrast_name': 'Total', 'flagged_scan_count': total_flags}])
...
def main():
    ...
    try:
        ...
        crit1 = (df['VIF'] >= thresholds['vif_combined']) & (df['image_outlier_percentage'] >= thresholds['outlier_combined'])
        ...
        print(f"Criteria: (VIF >= {thresholds['vif_strict']}) OR "
        ...
        if not flagged_df.empty:
            ...
            flagged_df['subject_num'] = flagged_df['subject_label'].str.extract(r'sub-s(\d+)').astype(int)
            flagged_df['session_num'] = flagged_df['session_id'].astype(int)
            
            sorted_df = flagged_df.sort_values(by=['subject_num', 'task_name', 'session_num'])

            final_columns = [
                'subject_label', 'task_name', 'contrast_name', 
                'image_outlier_percentage', 'VIF', 'flag_reason'
            ]
